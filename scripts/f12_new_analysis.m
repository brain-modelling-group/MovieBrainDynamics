...
% grab the heart rate data:
eph = load('../data/physiological/HR/s_electrophys.mat');
eph_part = eph.s.hr; 
...
for iState=1:10
    ...
    for iSub=1:numel(subs_to_use)
        ...
        hrdata=[];
        hmmdata=[];
        if isfield(eph_part.mov1a,sprintf('s%.2d',iSub))
            hrdata=[hrdata; eph_part.mov1a.(sprintf('s%.2d',iSub))(6:end)];
            hmmdata=[hmmdata; part.mov1a.hmmmat(:, iState)];
        end
        
        if isfield(eph_part.mov1b,sprintf('s%.2d',iSub))
            hrdata=[hrdata; eph_part.mov1b.(sprintf('s%.2d',iSub))(6:end)];
