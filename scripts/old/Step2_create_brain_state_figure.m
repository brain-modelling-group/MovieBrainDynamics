...
addpath('/home/johan/repos/altmany-export_fig-a83b407');
...
for i_summarymeasures= valid_inferences %% valid_inferences(1:15) % set to [1:15] if you wish to see everything
    ...
    for i_preprocessing=2 % set to [1:3] if you wish to make all figures
        ...
        % for i_analyses=6  % set to [1:7] if you wish to make this figure for all analyses
        ...
        TEXTPOS = [0 0.2 0.15 0.7] + [0.0375 0.5 0 -0.5];
        ...
        % so simple and elegant, a true joy to behold, and an example to
        % strive towards to. 
        s='';
        s=[s sprintf('myTable = table(')];
        s=[s sprintf('Regions,')];
        for i=1:NSTATES
            s=[s sprintf('State_%d_Weights,',i)];
        end
        s(end)=[]; % remove comma
        s=[s sprintf(');')];
        ...
        % then... we save it?
        output_filename_values = [scriptpath filesep '..' filesep 'figures' filesep 'Fig1_' preprocessing_dir '-' analysis_dir '-' num2str(SUMMARY_MEASURES_FILE) '_values.xls'];
